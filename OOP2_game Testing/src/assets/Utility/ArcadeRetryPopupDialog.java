...
/**
 * End-of-arcade-run retry popup with three buttons:
 *   Play Again → onYes
 *   Leaderboard → onLeaderboard  (does NOT close the dialog, shows leaderboard on top)
 *   Back to Menu → onBackToMenu
 */
public class ArcadeRetryPopupDialog extends JDialog {
    ...
    public ArcadeRetryPopupDialog(Window owner,
                                  Runnable onYes,
    ...
    /** Does NOT close the dialog — leaderboard opens on top and user returns here after. */
    ...
    private final class PopupPanel extends JPanel {
        ...
        private void handleMouseClick(int x, int y) {
            if      (yesButton         != null && yesButton.contains(x, y))         chooseYes();
            else if (leaderboardButton != null && leaderboardButton.contains(x, y)) chooseLeaderboard();
            else if (backButton        != null && backButton.contains(x, y))        chooseBackToMenu();
        }

        private void handleMouseMove(int x, int y) {
            boolean wasYes  = hoverYes;
            boolean wasLb   = hoverLeaderboard;
            boolean wasBack = hoverBack;
            ...
            if (wasYes != hoverYes || wasLb != hoverLeaderboard || wasBack != hoverBack) {
                repaint();
            }
        }
    }
