package engine;

import assets.Utility.FontManager;

import javax.imageio.ImageIO;
import javax.swing.*;
import java.awt.*;
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;
import java.awt.image.BufferedImage;
import java.awt.image.ConvolveOp;
import java.awt.image.Kernel;

public class PauseMenuPanel extends JPanel {
    private static final int SCREEN_WIDTH = 1536;
    private static final int SCREEN_HEIGHT = 896;

    private final GameWindow window;
    private final String previousPanelCard;  // Card name of the previous panel (GAME, SURVIVAL, ARCADE)
    private final Component previousComponent;
    private boolean hoverResumeButton = false;
    private boolean hoverMainMenuButton = false;
    private boolean hoverQuitButton = false;

    // UI positioning
    private Rectangle resumeButton;
    private Rectangle mainMenuButton;
    private Rectangle quitButton;

    private final Font titleFont = FontManager.getFont(38f).deriveFont(Font.BOLD);
    private final Font subtitleFont = FontManager.getFont(24f).deriveFont(Font.BOLD);
    private final Font bodyFont = FontManager.getFont(20f).deriveFont(Font.PLAIN);

    public PauseMenuPanel(GameWindow window, String previousPanelCard, Component previousComponent) {
        this.window = window;
        this.previousPanelCard = previousPanelCard;
        this.previousComponent = previousComponent;
        setPreferredSize(new Dimension(SCREEN_WIDTH, SCREEN_HEIGHT));
        setFocusable(true);
        setFocusTraversalKeysEnabled(false);

        addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                handleMouseClick(e.getX(), e.getY());
            }

            @Override
            public void mouseMoved(MouseEvent e) {
                handleMouseMove(e.getX(), e.getY());
            }
        });

        addMouseMotionListener(new MouseAdapter() {
            @Override
            public void mouseMoved(MouseEvent e) {
                handleMouseMove(e.getX(), e.getY());
            }
        });

        addKeyListener(new java.awt.event.KeyAdapter() {
            @Override
            public void keyPressed(java.awt.event.KeyEvent e) {
                if (e.getKeyCode() == java.awt.event.KeyEvent.VK_ESCAPE) {
                    resumeGame();
                }
            }
        });
    }

    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        Graphics2D g2 = (Graphics2D) g.create();
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);

        // Draw blurred and darkened background (capture previous component and blur)
        paintBackground(g2);

        // Draw pause menu panel (rectangular component in center)
        paintPauseMenu(g2);

        g2.dispose();
    }

    private void paintBackground(Graphics2D g2) {
        try {
            if (previousComponent != null) {
                int w = getWidth();
                int h = getHeight();
                BufferedImage buf = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
                Graphics2D ig = buf.createGraphics();
                ig.setColor(new Color(0,0,0,0));
                ig.fillRect(0,0,w,h);
                // paint previous component into image
                previousComponent.paint(ig);
                ig.dispose();

                // small 5x5 box blur
                int size = 5;
                float weight = 1.0f / (size * size);
                float[] data = new float[size * size];
                for (int i = 0; i < data.length; i++) data[i] = weight;
                ConvolveOp op = new ConvolveOp(new Kernel(size, size, data), ConvolveOp.EDGE_NO_OP, null);
                BufferedImage dst = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
                op.filter(buf, dst);

                // draw blurred background with slight darken
                Composite old = g2.getComposite();
                g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.85f));
                g2.drawImage(dst, 0, 0, null);
                g2.setComposite(old);

                // dark overlay to improve contrast
                g2.setColor(new Color(0, 0, 0, 120));
                g2.fillRect(0, 0, w, h);
                return;
            }
        } catch (Exception ignored) {}

        // fallback dark overlay
        g2.setColor(new Color(0, 0, 0, 150));
        g2.fillRect(0, 0, getWidth(), getHeight());
    }

    private void paintPauseMenu(Graphics2D g2) {
        // Compact layout: measure text and buttons and size panel tightly
        String title = "Game Paused";
        String resumeText = "Resume Game";
        String menuText = "Main Menu";
        String quitText = "Quit Game";

        FontMetrics titleFm = g2.getFontMetrics(titleFont);
        FontMetrics btnFm = g2.getFontMetrics(subtitleFont);

        int padX = 36; // horizontal padding inside panel
        int padY = 22; // vertical padding
        int btnPadX = 28; // horizontal padding inside button
        int btnPadY = 14; // vertical padding inside button
        int btnGap = 14; // gap between buttons

        int titleW = titleFm.stringWidth(title);
        int b1 = btnFm.stringWidth(resumeText.toUpperCase()) + btnPadX * 2;
        int b2 = btnFm.stringWidth(menuText.toUpperCase()) + btnPadX * 2;
        int b3 = btnFm.stringWidth(quitText.toUpperCase()) + btnPadX * 2;
        int buttonWidth = Math.max(Math.max(b1, b2), b3);
        int buttonHeight = btnFm.getHeight() + btnPadY * 2;

        int panelWidth = Math.max(titleW, buttonWidth) + padX * 2;
        int panelHeight = padY * 2 + titleFm.getHeight() + btnGap + buttonHeight * 3 + btnGap * 2;

        int panelX = (getWidth() - panelWidth) / 2;
        int panelY = (getHeight() - panelHeight) / 2;

        // subtle shadow
        g2.setColor(new Color(8, 6, 4, 100));
        g2.fillRoundRect(panelX + 6, panelY + 8, panelWidth, panelHeight, 14, 14);

        // Draw translucent earthy panel background (more opaque so it doesn't blend too much)
        Color panelBg = new Color(70, 55, 40, 230); // warm brown, more opaque
        g2.setColor(panelBg);
        g2.fillRoundRect(panelX, panelY, panelWidth, panelHeight, 14, 14);

        // subtle border
        g2.setColor(new Color(30, 24, 18, 170));
        g2.setStroke(new BasicStroke(2f));
        g2.drawRoundRect(panelX, panelY, panelWidth, panelHeight, 14, 14);

        // Draw title
        int titleX = panelX + (panelWidth - titleW) / 2;
        int titleY = panelY + padY + titleFm.getAscent();
        g2.setFont(titleFont);
        g2.setColor(new Color(245, 242, 238));
        g2.drawString(title, titleX, titleY);

        // Buttons: stacked compactly
        int buttonX = panelX + (panelWidth - buttonWidth) / 2;
        int startY = titleY + padY + btnGap;

        resumeButton = new Rectangle(buttonX, startY, buttonWidth, buttonHeight);
        paintButton(g2, resumeButton, resumeText.toUpperCase(), hoverResumeButton);

        mainMenuButton = new Rectangle(buttonX, startY + buttonHeight + btnGap, buttonWidth, buttonHeight);
        paintButton(g2, mainMenuButton, menuText.toUpperCase(), hoverMainMenuButton);

        quitButton = new Rectangle(buttonX, startY + (buttonHeight + btnGap) * 2, buttonWidth, buttonHeight);
        paintButton(g2, quitButton, quitText.toUpperCase(), hoverQuitButton);
    }

    private void paintButton(Graphics2D g2, Rectangle bounds, String text, boolean hovered) {
        // Draw button background - earthy palette
        Color base = new Color(115, 90, 60);
        Color hover = new Color(140, 110, 80);
        Color bgColor = hovered ? hover : base;
        g2.setColor(bgColor);
        g2.fillRoundRect(bounds.x, bounds.y, bounds.width, bounds.height, 10, 10);

        // subtle inner highlight
        GradientPaint gp = new GradientPaint(bounds.x, bounds.y, new Color(160,130,100,40), bounds.x, bounds.y + bounds.height, new Color(0,0,0,0));
        g2.setPaint(gp);
        g2.fillRoundRect(bounds.x, bounds.y, bounds.width, bounds.height / 2, 10, 10);
        g2.setPaint(null);

        // Draw button border
        g2.setColor(new Color(40, 30, 22, 180));
        g2.setStroke(new BasicStroke(2f));
        g2.drawRoundRect(bounds.x, bounds.y, bounds.width, bounds.height, 10, 10);

        // Draw centered button text (no icon)
        g2.setFont(subtitleFont);
        g2.setColor(new Color(246, 244, 240));
        FontMetrics fm = g2.getFontMetrics();
        int textX = bounds.x + (bounds.width - fm.stringWidth(text)) / 2;
        int textY = bounds.y + (bounds.height + fm.getAscent()) / 2 - 3;
        g2.drawString(text, textX, textY);
    }

    private void handleMouseClick(int x, int y) {
        if (resumeButton.contains(x, y)) {
            resumeGame();
        } else if (mainMenuButton.contains(x, y)) {
            goToMainMenu();
        } else if (quitButton.contains(x, y)) {
            quitGame();
        }
    }

    private void handleMouseMove(int x, int y) {
        boolean wasResumeHovered = hoverResumeButton;
        boolean wasMainMenuHovered = hoverMainMenuButton;
        boolean wasQuitHovered = hoverQuitButton;

        hoverResumeButton = resumeButton != null && resumeButton.contains(x, y);
        hoverMainMenuButton = mainMenuButton != null && mainMenuButton.contains(x, y);
        hoverQuitButton = quitButton != null && quitButton.contains(x, y);

        // Repaint only if hover state changed
        if (wasResumeHovered != hoverResumeButton || wasMainMenuHovered != hoverMainMenuButton || wasQuitHovered != hoverQuitButton) {
            repaint();
        }
    }

    private void resumeGame() {
        window.resumeFromPause(previousPanelCard);
    }

    private void goToMainMenu() {
        window.showMenu();
    }

    private void quitGame() {
        System.exit(0);
    }
}
